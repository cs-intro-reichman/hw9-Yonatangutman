...
public class LinkedList {
	...
	Node toEqual= first;
	while (toEqual.next != node && toEqual.next != null) {
		toEqual = toEqual.next;
	}
	if (toEqual.next == node) {
		toEqual.next = node.next;
		size--;
		if (node == last) {
			last = toEqual;	
		}
    ...
    public void remove(MemoryBlock block) {
		...
		Node currNode = first;
		while (currNode.next != null && !currNode.next.block.equals(block)) {
		...
		Node toErase = currNode.next;
		currNode.next = toErase.next;
		size--;
